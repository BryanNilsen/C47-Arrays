...
export const getStudentByLastName = (lastName) => students.find(student => student.lastName.toLowerCase() === lastName.toLowerCase())
...
export const getStudentByName = (fullName) => students.find(student => `${student.firstName.toLowerCase()} ${student.lastName.toLowerCase()}` === fullName.toLowerCase())
...
export const getInstructorOfStudent = (studentId) => instructors.find(instructor => instructor.id === students.find(student => student.id === studentId).instructorId)
...
export const getStudentWithMostLangs = () => {
    let languageCount = 0
    let studentWithMostLanguages = {}
    for (const student of students) {
        if (student.languages.length > languageCount) {
            languageCount = student.languages.length
            studentWithMostLanguages = student
        }
    }
